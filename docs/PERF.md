...
**生成命令**：`.\run-perf.ps1 -Scenario 3 -Loops 1000` ｜ **日期**：2026-09-02 ｜ **归档**：`results/20260902-171325`（s3-*.csv；当时脚本为原生采样器版本，无 TTFT 记录）

| 并发 | 采样数 | QPS | P99 ms | 最大 ms | 错误率 | 主要错误 |
|---|---|---|---|---|---|---|
| 50 | 50,000 | 275 | 809 | 10,036 | **1.36%** | BindException（客户端端口耗尽前兆） |
| 100 | 100,000 | 650 | 522 | 15,060 | **56.30%** | ConnectException: Connection refused |
| 200 | 200,000 | 1,299 | 281 | 15,133 | **78.09%** | ConnectException: Connection refused |
| 500 | 500,000 | 2,271 | 810 | 15,370 | **87.06%** | ConnectException: Connection refused |

> **优化前基线解读**：SSE 每轮对话 = 一条独占长连接（读完 END 才收尾），并发爬升时 **accept 队列被打满**，`Connection refused` 错误率从 1.36% 飙到 87.06%——这是典型的**连接容量瓶颈**而非计算瓶颈（LLM 为 mock 流式，CPU 占用低）。
>
> **已实施优化（2026-09）**：`Flux.defer` + `AtomicInteger` 连接级并发限流（`zx.rag.max-concurrent-streams`，默认 2000），超限返回降级流（"请稍后再试" + END）——把错误率转化为**显式降级**。
>
> **优化后**：`<X>`（需用当前 JSR223 版脚本重测回填；SSE 场景请用小并发 × 小循环，如 `-Threads "50,100" -Loops 5`，见 run-perf.ps1 注释）。

### 5.2 SSE 首 token 延迟（TTFT）

**生成命令**：`.\run-perf.ps1 -Scenario 3 ...`（当前 JSR223 脚本含 `TTFT-首token` 子采样）｜ **日期**：待重测

| 并发 | P50 | P95 | P99 | 备注 |
|---|---|---|---|---|
| 50 | `<X>` | `<X>` | `<X>` | LLM mock 流式模式（`ZX_LLM_ENABLED=false`） |
| 接真实 LLM | `<X>` | `<X>` | `<X>` | 受上游模型推理时延主导，仅作链路开销参考 |

> 2026-09-02 归档为旧版采样器，无 TTFT 记录，故本表全部占位。
